  ...
  <div class="py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
      ...
      <h1 class="font-sans text-3xl font-bold text-gray-900">
        Inserisci Nuovo Vino
      </h1>
      <p class="text-xs text-gray-500 mt-1">
        Popola i dettagli della bottiglia e la scheda tecnica dettagliata.
      </p>
    </div>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-8 bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
      ...
      <div v-if="isAdmin" class="bg-amber-50/70 p-5 rounded-2xl border border-amber-200/60">
        <label class="flex items-center gap-1.5 text-xs font-bold text-amber-900 mb-1 uppercase tracking-wider">
          ...
          <span>Selezione Cantina (Funzionalità Riservata Super Admin) *</span>
        </label>
        <p class="text-xs text-amber-800/80 mb-3">Come Admin puoi associare questo vino direttamente a qualsiasi cantina del Molise:</p>
        <select 
        ...
        >
          <option value="" disabled>-- Seleziona la Cantina Produttrice --</option>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
            <div class="sm:col-span-2">
              ...
              <input 
                ...
                type="text" 
                required 
                placeholder="es. Tintilia del Molise Riserva DOC" 
      ...
      <div class="bg-stone-50/70 p-6 rounded-2xl border border-stone-200/60">
        ...
        <div v-if="customAttributes.length" class="space-y-4">
          <div v-for="(attr, idx) in customAttributes" :key="idx" class="p-4 bg-white rounded-2xl border border-stone-200/80 shadow-xs space-y-3">
            ...
            <!-- Quick Presets / Range Chips (e.g. Gradazione, Temperatura, Vinificazione...) -->
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <textarea 
              ...
              rows="4" 
              placeholder="Racconta le caratteristiche del vino, il periodo di affinamento, la botte usata..." 
              class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-wine-800 focus:outline-none"
            ></textarea>
          </div>
        </div>
      ...
      <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <textarea v-model="form.tasting_notes.visual" rows="2" placeholder="Giallo paglierino con riflessi dorati..." class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-wine-800 focus:outline-none"></textarea>
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-700 mb-1">Esame Olfattivo</label>
            <textarea v-model="form.tasting_notes.olfactory" rows="2" placeholder="Fiori bianchi, pesca gialla, macchia mediterranea..." class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-wine-800 focus:outline-none"></textarea>
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-700 mb-1">Esame Gustativo</label>
            <textarea v-model="form.tasting_notes.taste" rows="2" placeholder="Fresco, sapido, equilibrato e minerale..." class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-wine-800 focus:outline-none"></textarea>
          </div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input 
              ...
              type="text" 
              placeholder="https://... oppure carica file sotto" 
...
const handleSubmit = async () => {
  ...
  const cleanVintageYear = (form.vintage_year !== '' && form.vintage_year !== null && form.vintage_year !== undefined)
    ? (isNaN(Number(form.vintage_year)) ? null : Number(form.vintage_year))
    : null

  const cleanAlcoholDegrees = (form.alcohol_degrees !== '' && form.alcohol_degrees !== null && form.alcohol_degrees !== undefined)
    ? (isNaN(Number(form.alcohol_degrees)) ? null : Number(form.alcohol_degrees))
    : null
