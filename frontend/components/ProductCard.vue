<template>
  <div class="bg-white rounded-2xl border border-stone-200/60 shadow-xs hover:shadow-md transition-all duration-300 flex flex-col overflow-hidden group">
    ...
    <div class="relative h-72 bg-white border-b border-stone-100 overflow-hidden flex items-center justify-center p-6">
      ...
      <div class="absolute top-4 right-4 z-10">
        <span :class="['px-2.5 py-1 text-xs font-semibold rounded-lg border shadow-xs transition-colors', getCategoryBadgeClass(product.category)]">
      ...
      <div>
        ...
        <div v-if="product.food_pairings && product.food_pairings.length" class="mt-4 flex flex-wrap gap-1.5">
          <span 
            ...
            :key="idx"
            class="inline-flex items-center space-x-1 text-xs bg-stone-100/80 text-stone-700 px-2.5 py-1 rounded-lg font-medium border border-stone-200/50"
          >
